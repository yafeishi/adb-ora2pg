...
sub _init
{
	...
	# Retreive all table information
		...
		$self->logit("global_where=".$self->{'global_where'}.",". "where=".%{$self->{'where'}}."\n");
...

{
	...
	my $sql_header = "-- Generated by Ora2Pg for ADB, the Oracle database Schema converter, version $VERSION\n";
...
BEGIN
          ...
          # end danghb
			...
			if ($self->{partitions_default}{$table}) {
				...
				my $idxsql = $self->_create_indexes_on_sub_table($table,$pname);
				$create_table{$table}{'index'} .= $idxsql;
        		# end danghb
				$create_table{$table}{table} .= "\n-- Create default table, where datas are inserted if no condition match\n";
				$create_table{$table}{table} .= "CREATE TABLE $pname () INHERITS ($table);\n";
				if( ($self->{distribute_by_mode} eq 'REPLICATION') || ($self->{distribute_by_mode} eq 'ROUNDROBIN') ){
					$create_table{$table}{table} .= "ALTER TABLE $pname  DISTRIBUTE BY $self->{distribute_by_mode};\n";
				}
...
CREATE TRIGGER $trig_name BEFORE INSERT OR UPDATE
  ...
  FOR EACH ROW EXECUTE PROCEDURE $fctname();
			 ...
			 if ($unique) {
			 # add by jiangmj3  在唯一索引列后面追加默认的分片键
	...
	elsif ($self->{file_per_table}) 
	{
		...
		if ($self->{file_per_table} && $pname) 
		{
			if( ! $self->{split_file} )
			{
				...
				#关闭文件句柄
				$self->close_export_file($fh);
				#Written 301 bytes to tmp_PART_MAX_aaa.sql
				$self->logit("Written " . length($data) . " bytes to $dirprefix$filename\n", 1);
			}
			else
			{
